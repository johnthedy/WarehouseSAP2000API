function [RecRatio,idxframe]=SAPSIMULATION(sparam,gparam,DL,LL,WL,EQset,Wlist,Clist,Plist)
...
gparam1=round(gparam(1)/100)*100;
gparam2=round(gparam(2)/100)*100;
gparam3=round(gparam(3)/100)*100;
gparam4=round(gparam(4)/100)*100;
gparam5=gparam(5);
gparam6=gparam(6);
...
CCoord=[];
Cx=[0 gparam2];
Cy=[round(linspace(0,gparam1,gparam5-1))];
Cz=[0 gparam3];
for i=1:length(Cx)
    for j=1:length(Cy)
        for k=1:length(Cz)
...
Gy=[round(linspace(0,gparam1,gparam5-1))];
...
UDWY=WL.*LoadLength.*9.8./1000;

d2=find(strcmp(idxframe(:,1),'GIR'));
d3=1:length(d2)/(gparam5-1):length(d2);

for i=1:length(d3)
    for j=d3(i):d3(i)+length(d2)/(gparam5-1)-1
        ...
        FrameObj.SetLoadDistributed(sprintf('%d',d2(j)), "WINDY", 1, 4, 0, 1, UDWY(i), UDWY(i));
    end
end
